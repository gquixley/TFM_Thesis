...
import define_map_theatre                 # separate python file for network and junction info
# Choose what mapping function you want: define_map_5050, define_map_park, define_map_mall, define_map_theatre
...
network = define_map_theatre.network
junction_info = define_map_theatre.junction_info
...
with open('params.txt') as file:
    ...
    for element in params:
        ...
        if element == 'velocity_model':
            if (not params[element] in ['Greenshields', 'Underwood', 'Bell-Shaped', 'Greenberg']) or (not isinstance(params[element], str)):
                error_info['params'] += 1
                error_message += 'Bad velocity model'

        if element == 'riemann_solver':
            if (not params[element] in ['LaxFriedrichs', 'HLL', 'Rusanov', 'Murman-Roe', 'HLLC', 'HLLE']) \
...
def velocity_model(road_section, density, derivative):
    ...
    elif velocity_model_id == 'Greenberg':

        # Greenberg

        u_c = road_section['vmax']/2
        d_m = road_section['dmax']
        density += 1e-2

        velocity_model_value = u_c*np.log(d_m/density)

        if derivative == 1:
            velocity_model_value = - u_c/density

    elif velocity_model_id == 'Underwood':

        # Underwood
        u_f = road_section['vmax']
        d_c = road_section['dmax']*0.0769

        velocity_model_value = u_f*np.exp(-density/d_c)

        if derivative == 1:
            velocity_model_value = -u_f*np.exp(-density/d_c)/d_c

    elif velocity_model_id == 'Bell-Shaped':

        u_f = road_section['vmax']
        d_c = road_section['dmax']*0.0769

        velocity_model_value = u_f*np.exp(0.5*(density/d_c)**2)

        if derivative == 1:
            velocity_model_value = (u_f*density*np.exp(0.5*(density/d_c)**2))/d_c**2
...
def compute_flux():
    ...
    for cell_id in range(0, len(CellFluxes)):
        ...
        if riemann_solver == 'LaxFriedrichs':
            ...
            CdL3 = np.array([CdL, velocity_model(network[road], CdL, 0), 0.05])
            CdR3 = np.array([CdR, velocity_model(network[road], CdR, 0), 0.05])
            PVL = CSVtoPV(CdL3)
            PVR = CSVtoPV(CdR3)
            ...
            FL = np.array([[DL*UL], [DL*UL**2+PL], [UL*(DL*EL+PL)]])
            FR = np.array([[DR*UR], [DR*UR**2+PR], [UR*(DR*ER+PR)]])

            # Compute guess pressure from PVRS Riemann solver
            PPV = max(0, 0.5*(PL+PR)+0.5*(UL-UR)*(0.25*(DL+DR)*(AL+AR)))
            Pmin = min(PL, PR)
            Pmax = max(PL, PR)
            Qmax = Pmax/Pmin
            Quser = 2.0
            ...
            if (Qmax <= Quser) and (Pmin <= PPV) and (PPV <= Pmax):
                ...
                if PPV < Pmin:
                    ...
                    GEL = np.sqrt((2/(gamma+1)/DL)/((gamma-1)/(gamma+1)*PL+PPV))
                    GER = np.sqrt((2/(gamma+1)/DR)/((gamma-1)/(gamma+1)*PR+PPV))
            ...
            S_L = UL-AL*ZL
            S_R = UR+AR*ZR
            S_M = (PL-PR + DR*UR*(S_R-UR)-DL*UL*(S_L-UL))/(DR*(S_R-UR)-DL*(S_L-UL))

            # Flux
            if 0 <= S_L:
                flux_value = FL[0]
                # flux_value = flux_val[0]
            elif S_L <= 0 <= S_M:
                qsL = DL * (S_L - UL)/(S_L - S_M) * 1  # np.array([[1], [S_M], [PVL[2] / DL + (S_M - UL) * (S_M + PL / (DL * (S_L - UL)))]])
                flux_value = FL[0] + S_L*(qsL - PVL[0])  # Check what FlL, S_L, CdL are
                # flux_value = flux_val[0]
            elif S_M <= 0 <= S_R:
                qsR = DR * (S_R - UR)/(S_R - S_M) * 1  # np.array([[1], [S_M], [PVR[2] / DR + (S_M - UR) * (S_M + PR / (DR * (S_R - UR)))]])
                flux_value = FR[0] + S_R*(qsR - PVR[0])
                # flux_value = flux_val[0]
            elif S_R <= 0:
                flux_value = FR[0]
            ...
            CdL3 = np.array([CdL + epsilon, velocity_model(network[road], CdL, 0), 0.05])
            CdR3 = np.array([CdR + epsilon, velocity_model(network[road], CdR, 0), 0.05])
            PVL = CSVtoPV(CdL3)
            PVR = CSVtoPV(CdR3)
            ...
            if DR or DL == 0:
                ...
                eta = 0.5*((np.sqrt(DL)*np.sqrt(DR))/(np.sqrt(DL)+np.sqrt(DR))**2)
                d_bar = ((np.sqrt(DL)*AL**2+np.sqrt(DR)*AR**2)/(np.sqrt(DL)+np.sqrt(DR)))+eta*(UR-UL)**2

            S_L = u_bar-d_bar  # velocity_model(network[road], CdL, 0) + CdL * velocity_model(network[road], CdL, 1)
            S_R = u_bar+d_bar  # velocity_model(network[road], CdR, 0) + CdR * velocity_model(network[road], CdR, 1)
            ...
            if S_L > 0:
                ...
                flux_value = (S_R * FL[0] - S_L * FR[0] + S_L * S_R * (PVR[0] - PVL[0])) / (S_R - S_L)
            elif S_R < 0:
                flux_value = FR[0]
...
def spatial_reco(reconstruction_type):
    ...
    for cell_id in range(0, len(output_array)):
        ...
        if reconstruction_type == 'FirstOrder':
            ...
            # 3rd-Order Weighted Essentially Non-Oscillatory reconstruction
...
for t in tqdm(np.arange(dt, T, dt)):  # loop with progress bar
    ...
    for road in network:
        ...
        for RK in range(0, 3):
            ...
            # Add in ghost BCs
            rho_ghost = np.append(rho_flux, [rho_flux[-2:-5:-1], rho_flux[3:0:-1]])  # What is this?!?!
            #print(rho_ghost)
...
if do_print:
    ...
    info_txt.write('{:30s} {:10s}\n'.format('Velocity Model', velocity_model_id))
    info_txt.write('{:30s} {:10s}\n'.format('Riemann Solver', riemann_solver))
    ...
    num_lines = {}
    files = ['main_MH.py', 'define_map_theatre.py', 'params.txt', 'MUSCLReconstruction.py', 'WENOReconstruction.py']
    for file in files:
        num_lines[file] = 0
